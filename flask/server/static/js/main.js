...
var fixedUrl = $("#json-data_locator").val();
var termsInfo = [], datesInfo = [], topics = [];
var termsReady = false, topicsReady = false;
$(document).ready(function () {
    ...
    d3.json(fixedUrl, function (data) {
        ...
        $(document).trigger("topicsReady");
        $(document).trigger("termsReady");
    })
});

$(document).on("termsReady",function () {
...
$(document).on("topicsReady", function () {
    ...
    for (var topic in topics) {
        ...
        for (var k = 0; k < Math.min(20, info.length); k++) {
